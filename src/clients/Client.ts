  ...
  implements ClientInterface
{
  ...
  async allDevicesReadyToStart() {
    return new Promise(resolve =>
      this.waitForAllDevicesReadyToStartOrTimeout(Date.now(), resolve),
    );
  }
  ...
  waitForAllDevicesReadyToStartOrTimeout(startTime: any, resolve: any) {
    if (this.allRegisteredDevicesAreReadyToStart()) {
      ...
      Date.now() - startTime >=
      Constants.DEFAULT_PROCESS_START_TIMEOUT_MILLISECONDS
    ) {
      throw new Error(
        `ERROR: Timeout, not all devices were ready to start the scenario.`,
      );
    } else {
      setTimeout(
        this.waitForAllDevicesReadyToStartOrTimeout.bind(
          this,
          startTime,
          resolve,
        ),
  ...
  async allDevicesReadyToFinish() {
    return new Promise(resolve =>
      this.waitForAllDevicesReadyToFinishOrTimeout(Date.now(), resolve),
    );
  }
  ...
  waitForAllDevicesReadyToFinishOrTimeout(startTime: any, resolve: any) {
    if (this.allRegisteredDevicesAreReadyToFinish()) {
      ...
      Date.now() - startTime >=
      Constants.DEFAULT_PROCESS_FINISH_TIMEOUT_SECONDS
    ) {
      throw new Error(
        `ERROR: Timeout, not all devices were ready to finish the scenario.`,
      );
    } else {
      setTimeout(
        this.waitForAllDevicesReadyToFinishOrTimeout.bind(
          this,
          startTime,
          resolve,
        ),
  ...
  private allRegisteredDevicesAreReadyToStart(): Boolean {
    ...
    let ready_to_start_ids = DeviceProcess.processesInState(
      Constants.PROCESS_STATES.ready_to_start,
    );
    return (
  ...
  private allRegisteredDevicesAreReadyToFinish(): Boolean {
    let registered_ids = DeviceProcess.registeredProcessIds();
    let ready_to_finish_ids = DeviceProcess.processesInState(
      Constants.PROCESS_STATES.ready_to_finish,
    );
    return (
      registered_ids.filter(registered_id => {
