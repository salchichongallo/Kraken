...
export abstract class SignalingClient {
  ...
  async readSignal(
    signal: string,
    timeout: Number = Constants.DEFAULT_TIMEOUT_MILLISECONDS,
  ) {
    return new Promise(resolve =>
      this.waitForSignalOrTimeout(signal, timeout, Date.now(), resolve),
    );
  }
  ...
  private waitForSignalOrTimeout(
    ...
    if (signal === this.inboxLastSignal()) {
      ...
      throw new Error(
        `ERROR: Signal timeout, did not receive signal: ${signal}`,
      );
    } else {
      setTimeout(
        this.waitForSignalOrTimeout.bind(
          this,
          signal,
          timeout,
          startTime,
  ...
  writeSignal(receiverInboxId: string, signal: string) {
    FileHelper.instance().appendTextToFile(
      signal,
      this.inboxFilePathForId(receiverInboxId),
    );
  }

  createInbox() {
    FileHelper.instance().createKrakenSupportFileIfDoesNotExist(
  ...
  inboxLastSignal(): any {
    try {
      let contentsOfInbox = FileHelper.instance()
        .contentOfFile(this.currentInboxFilePath())
        .trim()
        .split('\n');
