...
export class AAPT {
  ...
  apkInfo(apkPath: string): any {
    ...
    const aaptResult: string = execSync(
      `aapt dump badging ${apkPath}`,
    ).toString();
    return {
      apkLaunchActivity:
        this.extractApkLaunchActivityFromDumpBadging(aaptResult) || 'Not found',
      apkPackage:
        this.extractApkPackageFromDumpBadging(aaptResult) || 'Not found',
    };
  }
