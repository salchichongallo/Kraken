...
export class DependencyChecker {
  ...
  checkDependencies() {
    ...
    console.log(
      `Android SDK [${this.isAndroidSdkInstalled() ? installedText : notInstalledText}] (Required only for mobile testing - ANDROID_HOME)`,
    );
    console.log(
      `Android ADB [${this.isAdbInstalled() ? installedText : notInstalledText}] (Required only for mobile testing - ANDROID_HOME/tools and ANDROID_HOME/platform-tools)`,
    );
    console.log(
      `Android AAPT [${this.isAaptInstalled() ? installedText : notInstalledText}] (Required only for Kraken's info command - ANDROID_HOME/build-tools/:version)`,
    );
    console.log(
      `Appium [${this.isAppiumInstalled() ? installedText : notInstalledText}] (Required only for mobile testing)`,
    );
    console.log(
      `Java [${this.isJavaInstalled() ? installedText : notInstalledText}] (JAVA_HOME)`,
    );
    console.log('Done.');
  }
