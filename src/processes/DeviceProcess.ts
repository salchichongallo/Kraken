...
export abstract class DeviceProcess implements DeviceProcessInterface {
  ...
  protected async runWithArgs(args: string[]) {
    const cucumberProcess = spawn('node', args, {
      stdio: 'inherit',
    });
  ...
  apkInfo(): any {
    ...
    if (
      jsonMobileInfo.type &&
      jsonMobileInfo.type.toLowerCase() == 'multiple'
    ) {
  ...
  static directory(): string[] {
    ...
    let directoryContent = FileHelper.instance().contentOfFile(
  ...
  static processesInState(state: Number): Number[] {
    ...
    if (!FileHelper.instance().pathExists(filePath)) {
      return [];
    }
  ...
  registerProcessToDirectory() {
    ...
    FileHelper.instance().appendTextToFile(
      `${this.id}${Constants.SEPARATOR}${this.device}`,
      Constants.DIRECTORY_PATH,
    );
  }
}
